...
public $widgetPathFull;
	...
	{
		return esc_html__(
			"History In Content CTA Widget",
			"rds-global-history-cta-widget"
		);
	}
	...
	protected function register_controls()
	{
		...
		$this->start_controls_section("rds_global_historyCTA_widget", [
			"label" => esc_html__(
				"History In Content CTA Widget",
				"rds-global-history-cta-widget"
			),
		...
		if (!empty($this->allVariation) && count($this->allVariation) > 1) {
			$defaultVariation = isset($args["page_templates"]["history_page"]["in_content_cta"]["variation"]) ? $args["page_templates"]["history_page"]["in_content_cta"]["variation"] : "a";
	...
	protected function render()
	{
		...
		if (!empty($this->allVariation) && count($this->allVariation) > 1) {
			$args["page_templates"]["history_page"]["in_content_cta"]["variation"] = sanitize_text_field($settings["variation"]);
		}
