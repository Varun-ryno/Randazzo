...
if ( $data && is_numeric( $data['id'] ) ) {             
		...
		$this->widgetPath = "schedule-service";  //Edit
	...
	public function get_title()
	{
		return esc_html__(
			...
			"rds-template-schedule-service-widget"
		);
	}
	...
	protected function register_controls()
	{
		...
		$this->start_controls_section("rds_schedule_service_widget", [
			"label" => esc_html__(
				"Schedule Service Template Widget",
				"rds-schedule-service-widget"
			),
		]);

		if (!empty($this->allVariation) && count($this->allVariation) > 1) {
			$defaultVariation = isset($args["page_templates"]["schedule_service_page"]["variation"]) ? $args["page_templates"]["schedule_service_page"]["variation"] : "a";
